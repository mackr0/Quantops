"""Full FRESH-START for the EXP-A* experiment (2026-06-29).

Why this script exists
----------------------
ELEVENTH reset (2026-06-29). Cloned from full_fresh_start_2026_06_24.py and
carries the full prior lineage intact (funding-guard, RC1-RC11, the 06-17
hard-to-borrow universe gate, and the 06-24 broker/journal freshness invariant
+ durable journaling that eliminated the oversell / phantom / decomposition
class — p166 PLUG naked short, the phantom-equity incidents).

NEW this reset — the institutional-universe baseline ($10 min price + $5M min
dollar-ADV, operator-only / never auto-tuned). It lives in the `stocks` segment
(segments.py), so the rebuilt profiles inherit $10/$5M automatically — no
manifest change. Rationale: ~80% of the prior cohort's realized losses came
from sub-$10 names this universe now excludes (HTZ alone, a ~$2.50 stock, was
60%). This reset gives all 13 arms a clean $1M start on that universe. The three
new paper keys must be pasted into NEW_KEYS below and verified ACTIVE / $1M / 0
positions before --apply (step1_verify_keys gates it).
...
Run:
    cd /opt/quantopsai
    venv/bin/python full_fresh_start_2026_06_29.py            # dry-run
    venv/bin/python full_fresh_start_2026_06_29.py --apply    # execute
"""
...
NEW_KEYS = [
    ...
    ("A1", "6-29-acct-1", "PKVFLLFXSBSPBABCDKMA3QXXD4",
        "G57TWHfg2F55qYrbHb7Lx4TLiEx3fsYtWboBiQS5t7Fk"),
    ("A2", "6-29-acct-2", "PKLR2XGZLBIRKDJASMED4NOIL2",
        "7yXQ1RVAaXwBaqoYZ8NDXmssomRm2urGkbYgidaFqULq"),
    ("A3", "6-29-acct-3", "PKHF4WAWXOZMCFFX65KH5VNZH2",
        "6ygturL1EEFLCE2CBg6qGCEWjbz7ikipvwGyWeB9UpCd"),
]
...
def main() -> int:
    ...
    args = ap.parse_args()
    print("=" * 70)
    print(f"FULL FRESH-START 2026-06-29 / 11th reset (apply={args.apply})")
    print("=" * 70)
    if not args.apply:
        print(
