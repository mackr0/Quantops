"""Full FRESH-START for the EXP-A* experiment (2026-06-10).

Why this script exists
----------------------
Fourth contamination wave. After the 2026-06-09 fresh-start, the
first market-open cycle this morning surfaced 3 new bug classes
that the 06-09 architecture rewrite had not addressed (or had
shipped a regression in). All three now have prevention-level
fixes rather than visibility-only fixes.

Carrying forward all of 06-09's structural fixes (RC1-RC8) — see
the 2026-06-09 docstring for that history. NEW root causes from
the 06-10 market open:

  RC9   Bracket children weren't being stamped onto the entry
        row's protective_*_order_id columns. The SDK's
        submit_order returns the parent without populating
        .legs, even when order_class='bracket'. My pre-fix code
        read order.legs immediately, got None, didn't stamp the
        journal. The next ensure_protective_stops sweep saw no
        own_protective_ids → called _cancel_stale_other_orders →
        cancelled the bracket's live TP within ~5 minutes of
        every entry. Reproduced live this morning: pid 67 CCO
        bought 13:41:16; bracket TP cancelled 13:46:38. Fixed
        in commit 9a7bfec: submit path re-fetches with
        get_order(parent_id, nested=True) to populate .legs;
        and ensure_protective_stops checks the entry's parent
        order_class and skips bracket entries entirely.

  RC10  AI proposed multileg strategies (bull_put_spread, etc.)
        under action='OPTIONS' (single-leg only). Parser passed
        option_strategy through with no validation; executor
        rejected with "Unsupported option_strategy". Operator
        saw GATED · BLOCKED badges that didn't tell the AI to
        use MULTILEG_OPEN. Fixed in commit 4ba44c1: parser
        rejects multileg strategy_names at the parse layer
        (proposal never reaches executor or brain). Prompt
        tightened to say "OPTIONS (single-leg ONLY)" and route
        spreads explicitly to MULTILEG_OPEN.

  RC11  AI proposed off-chain strikes for single-leg options.
        INTC $115 call exp 2026-07-17 was the concrete case —
        Alpaca returned "asset not found" because that contract
        doesn't exist on the chain. Single-leg path had NO
        upstream snap_to_listed_contract validation (multileg
        had it). Fixed in commit 4ba44c1: parser calls
        snap_to_listed_contract on every OPTIONS proposal; snaps
        to the nearest listed contract within tolerance, or
        rejects if no contract is within tolerance.

The drift detection alert is still operational. The protective-
orders per-profile isolation from 06-09 (commit 8278b31) and the
bracket-order architecture (06-09 c59337e + 06-10 9a7bfec) make
the shared-account contention pattern structurally impossible.

With these specific bug classes prevented at their source, restart
from clean state.
...
NEW_KEYS = [
    # 2026-06-10 (fourth-reset rev) — fresh paper-trading Alpaca
    # accounts created 2026-06-10 (operator label suffix `.1`
    # distinguishes from this morning's 6-10-acct1/2/3 that are
    # being revoked at Alpaca.com). $1M each. Step1_verify_keys
    # auth-checks + confirms equity and 0 positions before any
    # destructive writes happen.
    ("A1", "6-10-acct1.1", "PKXOJHHLIG4UG3KIBZ7DEPZTUH",
        "7bHytUt5ms1YuGtoRdwe2xABa35HzUi4SRhgCwXJyEGs"),
    ("A2", "6-10-acct2.1", "PKEJBE5QICYJGODO7NDYU5LJMA",
        "HBdQUx8eStE7aSDLSBsnNoWVePmU3tUSRyTThc6YhiK3"),
    ("A3", "6-10-acct3.1", "PKAMFT3SVP57CAJWCOVS54BC4N",
        "4DQgDPJT4EqumEDYxZkmpNgjq7VQiTPAuWsBHSRbWGUH"),
]
...
def main() -> int:
    ...
    args = ap.parse_args()
    print("=" * 70)
    print(f"FULL FRESH-START 2026-06-10 (apply={args.apply})")
    print("=" * 70)
    if not args.apply:
        print(
