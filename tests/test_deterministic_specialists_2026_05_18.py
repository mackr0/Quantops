...
_FIRE_CASES = [
    ...
    ("slippage_high_caution",
     {"signal": "BUY",
      "slippage_estimate": {"total_bps": 45.0, "slippage_dollars": 120.0},
    ...
    ("wide_spread_caution",
     {"signal": "BUY",
      "slippage_estimate": {"total_bps": 20.0, "slippage_dollars": 55.0},
      "slippage_str": "exec cost ~20.0 bps ($55 on this order)"},
     "CAUTION", None),
    ("extreme_high_price_caution",
     {"signal": "BUY", "price": 1500.0}, "CAUTION", None),
    # Real empty-of-signal payload shapes: every reader returns a
    # multi-key dict even when it has nothing to say — the old
    # single-key fixture here is how the dead len(v)>1 carrier
    # heuristic hid from this suite (P1.4).
    ("multi_alt_data_silent",
     {"signal": "BUY",
      "alt_data": {
          ...
          "stocktwits_sentiment": {"covered": False,
                                    "message_count_7d": 0,
                                    "net_sentiment_7d": None},
