"""Full FRESH-START for the EXP-A* experiment (2026-06-24).

Why this script exists
----------------------
TENTH reset (2026-06-24). Cloned from full_fresh_start_pm_2026_06_17.py and
lands on commit 619fd9b — the broker/journal divergence-class fix (per-cycle
freshness invariant + durable journaling) that eliminated the recurring
oversell / phantom / decomposition class (p166 PLUG naked short, the phantom-
equity incidents). Carries the full prior lineage intact (funding-guard,
RC1-RC11, the 06-17 hard-to-borrow universe gate). The three new paper keys
below were verified ACTIVE / $1M / 0 positions before this clone was committed.
...
Run:
    cd /opt/quantopsai
    venv/bin/python full_fresh_start_2026_06_24.py            # dry-run
    venv/bin/python full_fresh_start_2026_06_24.py --apply    # execute
"""
...
NEW_KEYS = [
    ...
    # 2026-06-24 — new paper accounts, verified ACTIVE / $1M / 0 positions
    # before this clone was committed. Lands on commit 619fd9b (the
    # broker/journal freshness invariant + durable journaling).
    ("A1", "6-24-acct-1", "PKVLJUYMSKFQ5L6Z6RRWTLMZ24",
        "C4e29vaUmCSWajE4gGm6tLEuBngB5QD4N1imZZD2pYdG"),
    ("A2", "6-24-acct-2", "PKPKJ2YNRC6ODL25IOOTALYBXL",
        "DNVG3zjWfnnyKU6txB3yfTM8FGegZ6Dd5mxhFawizDRP"),
    ("A3", "6-24-acct-3", "PKXYQ2H53XIRE6G3NH53IQD4TT",
        "6Cx57PNudFomq5WEvQejv6VAt3AHriJsXTrqyktw49DK"),
]
...
def main() -> int:
    ...
    args = ap.parse_args()
    print("=" * 70)
    print(f"FULL FRESH-START 2026-06-24 / 10th reset (apply={args.apply})")
    print("=" * 70)
    if not args.apply:
        print(
