"""Full FRESH-START for the EXP-A* experiment (2026-06-19).

Why this script exists
----------------------
Tenth reset (first of 2026-06-19). Cloned from
full_fresh_start_pm_2026_06_17.py (carries the full lineage: 06-12
funding-guard, 06-17 universe-alignment gate, RC1–RC11, gap fixes).

The 06-17 cohort accumulated ~$187K of PHANTOM EQUITY across 10
profiles: a protective sweep re-armed sell orders on already-closed
UWMC positions, those filled as real broker shorts, and the per-profile
books DROPPED the resulting shorts — leaving the sell proceeds in equity
with no offsetting position (dashboard showed EXP-A3-700K +$34K; real
P&L was a ~-$11K loss). The 2026-06-18 remediation is live as of this
reset: (1) get_virtual_positions surfaces stock oversell shorts instead
of dropping them (order-id truth), (2) the closed-position netting
prevents the protective sweep from re-arming a flat position, and (3) a
book-integrity gate now runs before entries EVERY cycle and HALTS on any
broker-drift / decomposition divergence (replacing the after-trading,
email-only, wrong-profile-range audit_runner). This clean restart is the
first cohort with phantom equity made structurally detectable-and-halted
...
to proceed if any key fails auth, equity != $1M, or n_positions != 0.
  ...
  A1 = 6-19-acct-1 (paste key) — must verify $1M / 0 positions, mult=4
  A2 = 6-19-acct-2 (paste key) — must verify $1M / 0 positions, mult=4
  A3 = 6-19-acct-3 (paste key) — must verify $1M / 0 positions, mult=4

Run:
    cd /opt/quantopsai
    venv/bin/python full_fresh_start_2026_06_19.py            # dry-run
    venv/bin/python full_fresh_start_2026_06_19.py --apply    # execute
"""
...
NEW_KEYS = [
    ...
    # 2026-06-19 — PASTE THE THREE NEW PAPER-ACCOUNT KEYS HERE before
    # running. These placeholders intentionally FAIL step1_verify_keys
    # (auth error) so the reset cannot run against the 06-17 accounts
    # (which hold the corrupt/phantom positions being deleted). Each new
    # account must be funded $1M with 0 open positions, and the OLD
    # accounts deleted at Alpaca.com first.
    ("A1", "6-19-acct-1", "PKVON3FTSZ5SYEGKWWKPIJW64A",
        "FHYwnwv5w5CnGDiCtbmwbXtMUsU7737rAR1WBAopjeGH"),
    ("A2", "6-19-acct-2", "PKED6HFB2FAXR7XEYPQYZS4QNX",
        "7mWuQPR2buBKmSnUinayC1GYtjbJnmLDyyphGncSYSNq"),
    ("A3", "6-19-acct-3", "PK2VRZLY75GXNLGKJZYII5ZF3J",
        "Az5RxP9iqRkeML2xK7WnNJhvrdKocR1i6E4PQt3EjpyA"),
]
...
def main() -> int:
    ...
    args = ap.parse_args()
    print("=" * 70)
    print(f"FULL FRESH-START 2026-06-19 (apply={args.apply})")
    print("=" * 70)
    if not args.apply:
        print(
