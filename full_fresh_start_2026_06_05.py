"""Full FRESH-START for the EXP-A* experiment (2026-06-05).

Why this script exists
----------------------
Second contamination wave. After the 2026-06-04 fresh-start, drift
re-accumulated because four root-cause bugs were still live:
  RC1  pending_protective rows never transitioned to closed when
       the broker fired the protective stop
  RC2  reconciler forward-walk dead-ended when Alpaca GC'd an
       intermediate replace link (trailing stops get replaced N
       times as the trail bumps)
  RC3  protective placement was not idempotent; broker_coverage
       blips produced duplicate pending_protective rows
  RC4  FIFO walk for closing entry lots wasn't scoped by
       occ_symbol, so stock SELLs could consume option BUY lots
All four shipped 2026-06-05 (commits 177dcfd, 0f12712, 7802717,
2f70c5d) with regression tests pinning each contract. With the
code now stable, restart from clean state.
...
NEW keys to install (paper-trading Alpaca accounts created 2026-06-05
funded $1M each per docs/15 v2.1: A1 baselines, A2 ablations, A3
scale-tests):
  A1 = PK2V5C34MXVZAW245ZKVLWRNIK (6-5-acct1)
  A2 = PKQJEEFXYEF6ZIEQQC4PAYPH36 (6-5-acct2)
  A3 = PKEQYU2QLEN3ONOQB5V6NKCASU (6-5-acct3)

Run:
    cd /opt/quantopsai
    venv/bin/python full_fresh_start_2026_06_05.py            # dry-run
    venv/bin/python full_fresh_start_2026_06_05.py --apply    # execute
"""
...
NEW_KEYS = [
    ("A1", "6-5-acct1", "PK2V5C34MXVZAW245ZKVLWRNIK",
        "2uPJp6shZ8Z7QRd2JsGdK54ysLtzs536YRxw9VonAjX4"),
    ("A2", "6-5-acct2", "PKQJEEFXYEF6ZIEQQC4PAYPH36",
        "4sjUT6jEJBGahoWtzFHTBJSBDCnfkdYR9vU5sP8gL1Mq"),
    ("A3", "6-5-acct3", "PKEQYU2QLEN3ONOQB5V6NKCASU",
        "9rgFmPtcC4bHzCW3fvXhQ8WgxuXjGqNHambP9iPggf1t"),
]
...
def main() -> int:
    ...
    args = ap.parse_args()
    print("=" * 70)
    print(f"FULL FRESH-START 2026-06-05 (apply={args.apply})")
    print("=" * 70)
    if not args.apply:
        print(
