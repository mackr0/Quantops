...
- **Compounding learning surface.** Every cycle generates labeled training data on the system's own predictions in the exact feature distribution it makes future predictions in. At the measured cadence (~21,000 predictions per 2.5 weeks across the fleet) the meta-model accumulates on the order of 50,000 labeled rows in roughly **2 months** of operation. That asset cannot be bought.
