...
| `alt_data_cache.py` | 2026-05-20 — SQLite-backed cache for daily-cadence alt-data sources. `get_all_alternative_data` wraps 18 source calls through `cache_or_fetch(source, symbol, fetcher)`; cache hits return in microseconds; misses fall through to live fetch and populate the cache as a side effect. Earns most of its value on intra-cycle dedup (multiple same-segment profiles asking about the same screener-surfaced symbols within a 30-min window). Kill-switch via `ALTDATA_CACHE_ENABLED=0`. The morning pre-warmer that shipped on the same date was retired the same day (modules removed); see `docs/archive/2026-06-04-pre-audit/21_ALTDATA_PREMARKET_WARMUP.md` "Retirement rationale" for why. |
...
| `book_fit.py` | Per-profile (OWN-book) concentration signal: max return-correlation of a candidate to the profile's own holdings + same-sector count, surfaced as a `PORTFOLIO FIT` line in the AI prompt; plus an own-book sector-concentration haircut applied in `_rank_candidates` so the AI's menu leads with diversifying names. Reads only `ctx.db_path` — no cross-profile data. (Replaced the old cross-profile concentration cap module, removed 2026-06-30 for violating profile isolation.) |
...
| `provider_circuit.py` | Per-(provider, model) circuit breaker for AI provider calls. 3 consecutive 5xx/timeout failures OPEN that model's circuit for 5min (exponential backoff to 30min); other models on the same provider stay closed. `ai_providers.call_ai` routes to the fallback chain when the primary's circuit is open. **Chain head = the operator-configured Fallback LLM** (`_resolve_operator_fallback_model`, from `users.llm_provider`/`llm_model` + the Settings Fallback LLM Key) — this may be ANY provider (incl. Anthropic/Claude); it is a deliberate operator choice and is honored regardless of provider. **The config-level (env-key) cross-provider candidates that follow still suppress Anthropic by default** — profiles configured for Gemini/OpenAI never *silently* route to paid Claude; opt that path in via `AI_ALLOW_ANTHROPIC_FALLBACK=1`. The two are distinct: the spend gate blocks only the silent env-key path, never the operator's explicit pick. |
...
Source: `tests/`. 464 test files covering:
...
- 5,640 tests, zero skipped (runtime `pytest.skip()` guards were systematically removed 2026-06-24; a structural guard now blocks broad-`except`→`skip`).
