...
| `alt_data_cache.py` + `altdata_warmup.py` + `premarket_warmup.py` | 2026-05-20 — SQLite-backed cache for daily-cadence alt-data sources. `get_all_alternative_data` wraps 18 source calls through `cache_or_fetch(source, symbol, fetcher)`; cache hits return in microseconds. `altdata_warmup.run_warmup()` pre-fetches the active universe (read from `cycle_data_*.json` shortlists) for every cacheable source. `premarket_warmup.py` is the cron entrypoint installed at 08:00 UTC weekdays. Kill-switch via `ALTDATA_CACHE_ENABLED=0`. Cycle cold-start ~10min → ~30-60sec when the cache is warm. See `docs/21_ALTDATA_PREMARKET_WARMUP.md`. |
