...
| `test_viewer_cannot_mutate_admin_state::test_every_mutating_endpoint_is_admin_required` | Every `@views_bp.route(..., methods=["POST"\|"PUT"\|"DELETE"\|"PATCH"])` must carry `@admin_required`. Without this guard, a viewer (read-only account linked to an admin) can mutate state on the admin's account — the 2026-05-07 kill-switch incident: viewer could POST to `/api/kill-switch` and freeze the admin's entire trading book. Endpoints intentionally writable by viewers (none today) must be added to `INTENTIONALLY_VIEWER_WRITABLE` with a written rationale. |
