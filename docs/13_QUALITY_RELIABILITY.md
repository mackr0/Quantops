...
1. **Pre-commit / CI tests** (7,026 tests, zero skipped — runtime `pytest.skip()` guards were systematically removed 2026-06-24, and a structural guard blocks broad-`except`→`skip` from silently disabling a test) — must pass before merge.
...
0b. **Cash door** — the oversell door's mirror, at the same choke point (`assert_buy_within_own_cash()` in `order_guard.py`, 2026-07-15): a stock BUY that adds long exposure is refused before the broker sees it when its estimated cost (limit price, or latest trade padded by a slippage reserve) exceeds the profile's **own** virtual cash (`get_virtual_cash()`), less the cash reserved for resting cover stops. Fails closed when cash or the reference price cannot be read. Never gates exits: protective covers bypass it, and a buy that covers an own-journal short is cash-checked only for the portion beyond the short. Option cash floors live in the option executors.
...
4. *(removed 2026-06-30)* ~~Cross-profile concentration cap~~ — it violated profile isolation (one profile's holdings blocking another's trade). Concentration is now steered, not gated: an advisory own-book `PORTFOLIO FIT` line and a sector-concentration rank haircut (`book_fit.py`).
...
- **`integrity_audit.audit_equity_identity`** — per-profile self-consistency: `equity == initial_capital + Σ(realized) + Σ(unrealized)`. The journal's own algebra. Built so that only a real bookkeeping disagreement can move it: both sides read ONE positions snapshot, so mark terms cancel and moving prices cannot produce drift (2026-07-24); realized is **leg-derived** — `compute_leg_realized()`, fill-true FIFO on the same price basis as `get_virtual_cash()` — so a pending exit's decision-vs-fill spread cannot either (2026-08-24), and corruption of the stored `pnl` column surfaces as its own `pnl_column` finding instead of masquerading as equity drift. Every row convention a writer introduces must therefore be taught to all three ledger lenses (positions, cash, realized) together: on 2026-09-19 the first worthless option expiries of Experiment 2 raised this alarm on penny-exact books because `$0` broker-activity closes were counted by cash but skipped by the realized FIFO — fixed, with a lockstep test pinning that every side cash counts, realized handles. Reads fail closed: an unreadable ledger returns `None` and the audit reports `errored`, never a fabricated zero (2026-07-27 sweep). Catches FIFO mismatches, hidden cash flows, market_value/unrealized_pl divergence — bugs that don't appear in broker comparisons because the broker is fine; the journal is internally inconsistent. Tolerance: $1.
