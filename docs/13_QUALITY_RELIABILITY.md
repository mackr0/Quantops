...
1. **Pre-commit / CI tests** (6,985 tests, zero skipped — runtime `pytest.skip()` guards were systematically removed 2026-06-24, and a structural guard blocks broad-`except`→`skip` from silently disabling a test) — must pass before merge.
