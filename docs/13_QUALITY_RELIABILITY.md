...
1. **Pre-commit / CI tests** (459 files, 5,593 tests, 3 environment-dependent skips — an `_EMPTY_FIRE_EXEMPT` rule whose purpose IS to fire on minimal context, a segment-context builder that needs a production-like env, and the strategy-zombie check which needs accumulated prediction data) — must pass before merge.
