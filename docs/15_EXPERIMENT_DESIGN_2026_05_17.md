...
**How each ablation is enforced (no upstream contamination).** A flag that only hides a component from the AI's prompt is not a real ablation — the component can still shape the candidate pool the AI selects from, or leak through a shared cache, so the arm "benefits from the thing it's supposed to lack." Each ablation is therefore enforced at every layer the component touches:

- **Shared screener pool is a deliberate common substrate, not a confound.** The price/volume/momentum/RSI screener and the hard-to-borrow filter read *no* ablation flag and hand every same-market profile an identical tradable universe. Arms converge here by design (it's the same starting universe for everyone), not because of a leak.
- **`enable_alt_data` is gated at the candidate pool, not just the prompt.** Four strategies source alt-data (`insider_cluster`, `insider_selling_cluster`, `short_squeeze_setup`, `analyst_upgrade_drift`) and their scores rank symbols into the AI's top-15. They declare `USES_ALT_DATA = True`, and `get_active_strategies` drops them for `enable_alt_data=0` profiles — so the No-Alt-Data arm's *pool* is built from price/volume/technical signals only, matching its blanked prompt. (A structural test pins that any strategy importing an alt-data source declares the marker.)
- **The shared specialist-ensemble cache is content-keyed, not segment-keyed.** Specialist verdicts (and risk VETOs) are cached on `segment + a hash of the actual ensemble input` (candidate set + alt_data + disabled specialists + model). Same-config profiles still share the result; an ablation arm whose input differs (blanked alt_data, meta-pregated symbol set) computes its own verdicts instead of inheriting the first concurrent caller's. Before 2026-06-18 this cache was keyed on `segment` alone and leaked one arm's verdicts to every same-segment arm.
- **`enable_self_tuning` / `enable_options`** never enter the candidate pool (self-tuning only annotates rows; options only add a multi-leg block to the prompt), so they need no pool-level gate.
