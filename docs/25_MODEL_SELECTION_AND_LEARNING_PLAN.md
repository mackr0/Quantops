...
| 2026-09-20 | 4.5 | **Batches 1–3 were not a fair test of the owned model.** Reading their artifacts on the Mac found the training driver never masked the prompt (the answer was 0.4–0.7% of the loss) and that 31–40% of training examples were longer than the 8,192-token window, so the trainer cut off their answers. The exam scores stand (the exam saw full prompts and graded real answers) and nothing was ever promoted or hosted, but "HOLD dominance" as the cause of the poor scores is now a hypothesis, not a finding. The batch-4 recipe is built and tested — masking always on, over-length examples split or dropped and never truncated, a refuse-to-train guard, target hygiene, train-split rebalancing with label origins, warmup + cosine learning rate, checkpoint sweep, guessing baselines with a paired test and an explicit promotion bar — and the Learning page now states the owned model's standing in the app. See [28](28_FINETUNE_BATCH4_BUILD_SPEC.md) §0 and [27](27_FINETUNE_TRAINING_LOG.md) | this commit |
