...
Replace the apex AI's per-cycle call (currently `gemini-2.5-flash-lite` via `ai_providers.call_ai`) with a fine-tuned `gpt-4o-mini` model trained weekly on this system's own resolved predictions. Cadence is incremental — each Sunday's training extends the prior week's checkpoint with the new week's archived predictions, rather than retraining from scratch on a giant corpus. Expected monthly cost at the measured prediction rate (~2K/profile/month, ~13 AI profiles): **~$15-30/month for OpenAI training + the small inference markup**, vs current Gemini steady-state spend ~$8/month at the observed $0.27/day rate (one full trading day on 2026-06-04). The training-spend dwarfs the inference savings; the lift case has to come from prediction-quality improvement, not cost arbitrage.
