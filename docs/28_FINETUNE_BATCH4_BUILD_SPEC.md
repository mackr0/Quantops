...
- [ ] **P13 — Batch 4 trained** on the Mac. *(Launched 2026-09-20, 1,200 steps; log `~/Quantops-finetune/train_batch4.log`.)*
...
- [ ] **P16 — Merged, pushed, deployed**; the Learning-page panel verified on prod.
...
**What a run can see.** Training throughput on the M2 Max is about 230 tokens per second (batch 3: ~31 seconds per step at ~7,000 tokens), so a 1,200-step run takes roughly 8–10 hours and sees about 1,200 examples — a fraction of the corpus, not a pass over it. Gradient checkpointing stays on. The checkpoint sweep, not the step count, picks the model.
