...
- [x] **P13 — Batch 4 trained** on the Mac — to step ~630 of 1,200, where macOS killed the job (see P13a); examined as it stands. *(Launched 2026-09-20, 1,200 steps; adapter `~/Quantops-finetune/adapters/20260920_171835`, log `~/Quantops-finetune/train_batch4.log`. The trainer's own run record shows `mask_prompt: True` and the schedule read as numbers; the length guard found 0 of 15,976 train and 0 of 215 val examples over the window. Measured: ~26 s per step and ~18 min per 100-example validation pass, so ~12–13 hours in all. After 20 steps it had trained on 568 tokens — answers only — where batch 3 trained on ~7,000 prompt tokens per step.)*
- [x] **P13a — Training ended early, at step ~630 of 1,200 (2026-09-21 ~03:00 UTC), killed by macOS, not by the recipe:** `[METAL] Command buffer execution failed: Impacting Interactivity` — the OS kills a long GPU job when the display needs the GPU. Checkpoints 100–600 are intact. Validation loss (answers only, 100 examples): 3.616 at start, then 0.595 / 0.605 / 0.625 / 0.748 / 0.548 / 0.673 at steps 100–600 — flat and noisy after step 100, no downward trend, so the run was **not resumed before examining it**: if no checkpoint shows promise, more steps of the same recipe are not the missing ingredient; if one does, training resumes from step 600 (`--resume-adapter`) and only the new checkpoints need examining, because the base's answers are kept on disk.
- [x] **P13b — The exam is resumable** (§4.12): every answer is written to disk as it is produced; a relaunch generates only what is missing. Launched 2026-09-21 03:10 UTC under a retry loop (`~/Quantops-finetune/run_exam_batch4.sh`, log `eval_batch4.log`): untrained base + checkpoints 100, 200, 500, 600 over 426 prompts / 613 graded decisions / 160 distinct stock-days. This exam's guessing baselines: always-HOLD 59.1%, majority class 59.1%, frequency-matched 45.5% (5th–95th percentile 42.4–48.6%).
- [x] **P14 — Batch 4 examined**: sweep run on the full holdout, ten raw generations read, doc 27 entry and verdict written, `finetune/status.json` updated. *(2026-09-21: the exam completed on its first attempt, 03:10 → 14:16 UTC. **NOT PROMOTABLE — no checkpoint clears the bar:** base 40.8%; step 100 14.5%, step 200 46.8%, step 500 — the lowest-validation-loss pick — 36.0%, step 600 43.7%; guessing band tops out at 48.6%, always-HOLD 59.1%. Steps 200/500/600 are each worse than the base on both directional classes. The raw answers show the adapter emits exactly one bare trade with a swinging action prior — all BUY at step 100, no BUY at step 600. Per §7.3 the next changes are named in doc 27's batch-4 entry — gradient accumulation and the missed-move cap, with own-entry discrimination reported by the exam — and must be built before batch 5; there is no retrain on this recipe.)*
...
### 4.12 The exam survives a crash — `_generate_answers()`, `generation_cache_path()` in `finetune/local_train.py`

*Failure answered:* an exam is about half a day of GPU time and wrote its results only at the very end, while macOS can kill a long Metal job at any moment — it ended batch 4's training run at step ~630 (`Impacting Interactivity`). A crash at hour eleven would have cost the whole exam.

- Every answer is appended to `<corpus>/generations/<answerer>__<digest>.jsonl` and fsynced the moment it is produced; a rerun loads what is there and generates only the missing prompts (a torn final line is dropped and regenerated, never trusted). A finished answerer is not even loaded into memory again.
- The file name carries a digest of the exam's prompts, the base model and the generation cap, so answers are only ever reused for the **same exam under the same settings**; rebuild the corpus and the old files are simply never looked at. The answerer label is `base`, or the checkpoint step **and its training run** — two runs' step-500 checkpoints never share a file. The hindsight labels are deliberately not part of the identity: relabeling does not invalidate what a model said.
- Consequence: the untrained base — the slowest answerer, because it writes long answers — is generated **once per exam**, however many sweeps follow (a resumed training run, a different `--sweep-top`).
- Run an exam under a retry loop (`~/Quantops-finetune/run_exam_batch4.sh` is the pattern): a killed attempt relaunches and continues.
...
- Crash survival: a generation crash costs one answer, not the exam (the rerun generates only the rest, in order); a finished answerer is neither regenerated nor loaded; a torn last line is regenerated, not trusted; answers are never reused for a different exam, model, generation cap or training run — and relabeling does not invalidate them.
