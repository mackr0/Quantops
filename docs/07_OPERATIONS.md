...
Every 30 minutes a growth report gives two views — **THIS WINDOW** (the last 30 minutes) and **SINCE START**. Read the window view for a leak: the scheduler imports its heavy modules lazily, on the first trading cycle, so the since-start view carries that one-time burst forever, while a leak is whatever keeps appearing window after window. Each view names the object **types** whose live count grew most, the **module-level containers** whose length grew most, and live counts of the usual suspects (DataFrames, connections, threads, HTTP clients). If RSS grew (by more than 30MB in a window, or 100MB since start) while object counts, buffer-holding types and containers all stayed quiet, it says the memory is **not** held by Python objects — look at the allocator or a C extension instead of a Python cache.

Each report also states how many MB pandas frames actually hold (each underlying buffer counted once) and, once the process has grown 100MB, runs `malloc_trim(0)` and logs RSS before and after (`grep "allocator trim"`): a large drop means the memory had already been freed by Python and was only being kept by the allocator; almost no drop means something native is still holding it. The first live session (2026-09-21) showed +344MB in one 30-minute window with Python objects almost flat — native memory, not a Python cache.
