"""Full FRESH-START for the EXP-A* experiment (2026-06-10, PM rev).

Why this script exists
----------------------
Second reset of 2026-06-10 (fifth overall). The morning reset
(full_fresh_start_2026_06_10.py) installed the `.1`-generation
account keys and restarted the experiment. The operator directed
another clean restart this afternoon, after the sync.sh
silent-deploy-failure class was discovered and hardened (see
CHANGELOG 2026-06-10 afternoon: "Both services running." had been
printed while prod ran stale code, so post-fix cycles were not
provably exercising the deployed fixes).

Carries forward unchanged: all RC1-RC8 structural fixes from the
2026-06-09 script and RC9-RC11 from the morning 06-10 script (see
full_fresh_start_2026_06_10.py's docstring for that history), plus
the 06-04 gap fixes (step 9 full 7-day altdata-log sweep, step 9b
non-ok scrape_runs clearing).

Restart from clean state on fresh `.2`-generation paper accounts.
...
to proceed if any key fails auth, equity != $1M, or n_positions != 0.
  ...
  A1 = 6-10-acct1.2 (PK2RGJ6Z...)
  A2 = 6-10-acct2.2 (PKZ7OSEI...)
  A3 = 6-10-acct3.2 (PKPUJV3Y...)

Run:
    cd /opt/quantopsai
    venv/bin/python full_fresh_start_pm_2026_06_10.py            # dry-run
    venv/bin/python full_fresh_start_pm_2026_06_10.py --apply    # execute
"""
...
NEW_KEYS = [
    # 2026-06-10 (fifth-reset / PM rev) — fresh paper-trading Alpaca
    # accounts (`.2` generation; the morning's `.1` accounts are
    # being deleted at Alpaca.com so nothing holds positions outside
    # our system). $1M each. Step1_verify_keys auth-checks +
    # confirms equity and 0 positions before any destructive
    # writes happen.
    ("A1", "6-10-acct1.2", "PK2RGJ6ZCZEKKERASBFDYN7YVM",
        "HM4xWiSnE1UNatYerG23AhjhTiAhxdW15TZoS86DGJ4z"),
    ("A2", "6-10-acct2.2", "PKZ7OSEIDXSSOTWXR7FI57CBSI",
        "DLWGCCA5Eo8BiHE4TUnTbiA5Sijs3Qe83pogJBcS4YNq"),
    ("A3", "6-10-acct3.2", "PKPUJV3Y7DCTKF5QAGQ2IW6HDE",
        "9LcsmuGvgwkiBKCYLssFJkuR4BKFGqmaqrJWELLvZNh6"),
]
...
def main() -> int:
    ...
    args = ap.parse_args()
    print("=" * 70)
    print(f"FULL FRESH-START 2026-06-10 PM rev (apply={args.apply})")
    print("=" * 70)
    if not args.apply:
        print(
