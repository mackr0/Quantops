"""Full FRESH-START for the EXP-A* experiment (2026-06-17).

Why this script exists
----------------------
Eighth reset. Cloned from full_fresh_start_pm_2026_06_12.py (the
canonical PM-rev lineage) — NOT 06_10, so the 06-12 funding-guard
work is carried forward intact (account_funding_guard per-cycle
broker-equity HALT, pre-market broker_accounts_funded smoke check,
certify_books check 0 BROKER FUNDING).

The 2026-06-16/17 run carries experiment-validity scars from a heavy
fix campaign: the per-profile order-isolation rebuild (A0–A3), the
stop-loss-protection restoration, the option-orphan backstop +
source fixes, and the 2026-06-17 multileg buy-to-close realized-P&L
state-machine fix (commit 5b96742). Those fixes are deployed and
green in tests, but the live books accumulated drift from the
pre-fix cycles (naked-stop leftovers from a broker-API timeout
burst, mixed pre/post-fix data). Rather than repair that data, the
operator chose a clean restart so every arm's data is born under
the fully-fixed regime and the new fixes can be judged on clean
cycles.
...
to proceed if any key fails auth, equity != $1M, or n_positions != 0.
  ...
  A1 = 6-17-acct-1 (paste key) — must verify $1M / 0 positions, mult=4
  A2 = 6-17-acct-2 (paste key) — must verify $1M / 0 positions, mult=4
  A3 = 6-17-acct-3 (paste key) — must verify $1M / 0 positions, mult=4

Run:
    cd /opt/quantopsai
    venv/bin/python full_fresh_start_2026_06_17.py            # dry-run
    venv/bin/python full_fresh_start_2026_06_17.py --apply    # execute
"""
...
NEW_KEYS = [
    ...
    ("A1", "6-17-acct-1", "PK4LXCWZEX2WN4M2R5C74AAUSZ",
        "DandskJ65C2cTJhZJftybEzWuDX6chDkouX6XByMBqc8"),
    ("A2", "6-17-acct-2", "PKVQ3OTFRZ6NKAJNBZJP6STZQT",
        "DkpmpNnzTjfy4oCoawdTQyDn1CaKqr4T3LuUceukpp5z"),
    ("A3", "6-17-acct-3", "PKCUGEVO5TOXF3VZOHUKYMO7XM",
        "DMpTS2tNabarPJUT44SHyY88fzXA7NkoWaBctUmh7HBo"),
]
...
def main() -> int:
    ...
    args = ap.parse_args()
    print("=" * 70)
    print(f"FULL FRESH-START 2026-06-17 (apply={args.apply})")
    print("=" * 70)
    if not args.apply:
        print(
